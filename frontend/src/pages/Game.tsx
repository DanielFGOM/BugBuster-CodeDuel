...
export default function Game() {
  ...
  const submitCode = async () => {
    ...
    try {
      ...
      setResult(res.data);
      if (res.data.success) {
        toast.success('¡Código correcto! El Bug ha sido eliminado.');
      } else {
  ...
  return (
    <div className="h-screen bg-[#F3F1EC] text-[#1E2233] flex flex-col font-['Inter'] overflow-hidden">
      ...
      {/* TOP BAR - Elegante y Limpia */}
      <header className="h-16 bg-white border-b border-[#E7E4DC] flex justify-between items-center px-6 shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold tracking-tight text-[#2E4A78] font-['Space_Grotesk']">
            BugBuster <span className="text-xs font-normal text-slate-400 ml-2 uppercase tracking-widest">Agent Terminal</span>
          </h1>
        </div>
        <button 
          onClick={logout} 
          className="text-xs font-bold uppercase tracking-widest bg-[#E7E4DC] hover:bg-red-100 hover:text-red-600 px-4 py-2 rounded-lg border border-transparent transition-all text-[#8A8A94]"
        >
          Cerrar Sesión
        </button>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* SIDEBAR - Misiones en Azul Profundo */}
        <aside className="w-72 bg-[#2E4A78] text-white flex flex-col shadow-xl">
          <div className="p-6 border-b border-white/10 bg-[#1E304F]">
            <h3 className="text-xs font-bold text-[#F5CFA3] uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 bg-[#F0973D] rounded-full animate-pulse"></span>
              Mission Log
            </h3>
          </div>
            ...
            {levels.map(level => (
              <div 
                ...
                className={`group cursor-pointer p-4 rounded-xl border transition-all duration-200 ${
                  ...
                  ? 'bg-[#F0973D] border-[#F0973D] text-[#17233B] shadow-lg scale-105' 
        ...
        <main className="flex-1 flex flex-col p-8 gap-6 overflow-hidden relative">
          {currentLevel ? (
            <>
              {/* Tarjeta de Misión - Estilo Limpio */}
              <div className="bg-white border border-[#E7E4DC] p-6 rounded-2xl shadow-sm animate-in fade-in slide-in-from-top-4 duration-500 relative overflow-hidden">
                <div className="absolute left-0 top-0 w-2 h-full bg-[#F0973D]"></div>
                <h2 className="text-2xl font-bold text-[#2E4A78] mb-2 font-['Space_Grotesk']">
                  Misión: {currentLevel.title}
                </h2>
                <p className="text-[#8A8A94] leading-relaxed max-w-3xl text-sm">
              ...
              <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
                ...
                <div className="flex flex-col gap-4 min-h-0">
                  ...
                  <div className="flex gap-3 shrink-0">
                    <button 
                      onClick={submitCode} 
                      className="flex-1 bg-[#2E4A78] hover:bg-[#1E304F] text-white font-bold py-3 rounded-xl transition-all transform active:scale-95 shadow-md uppercase tracking-wider font-['Space_Grotesk']"
                    >
                      Ejecutar Código
                    </button>
                    <button 
                      onClick={() => setCode(currentLevel.template.replace('//USER_CODE', ''))} 
                      className="px-6 bg-white hover:bg-gray-50 text-[#8A8A94] rounded-xl border border-[#E7E4DC] transition-all shrink-0 font-semibold"
                    >
                      Reset
                    </button>
                  </div>
                  ...
                  {result && (
                    <div className={`p-4 rounded-xl border animate-in zoom-in-95 duration-300 ${
                      ...
                      <div className="flex items-center gap-2 mb-1 font-bold uppercase text-xs">
                        {result.success ? '✅ Éxito' : '❌ Error de Sistema'}
                      </div>
                ...
                <div className="flex flex-col gap-6 min-h-0">
                  <div className="flex-1 bg-white rounded-2xl border border-[#E7E4DC] shadow-sm relative overflow-hidden group">
            ...
            <div className="flex-1 flex items-center justify-center text-[#8A8A94] animate-pulse font-['IBM_Plex_Mono']">
