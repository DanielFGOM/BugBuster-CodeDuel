  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f5f7f8] p-6 font-['Plus_Jakarta_Sans',_sans-serif]">
      <div className="w-full max-w-[1000px] min-h-[590px] grid grid-cols-1 lg:grid-cols-2 bg-white rounded-[24px] overflow-hidden shadow-2xl">
        ...
        {/* PANEL IZQUIERDO */}
        <div className="relative bg-gradient-to-br from-[#1B2A41] to-[#101c2c] text-white p-12 z-10" 
          ...
          <div className="pr-20 h-full flex flex-col">
            ...
            <h1 className="text-3xl font-extrabold leading-tight mb-12 max-w-[320px]">
            ...
            <div className="flex justify-between w-full mb-12 relative">
              {steps.map((step, idx) => (
                <div key={idx} className="flex flex-col items-center z-10">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-500 ${
                    idx <= currentStep ? 'bg-[#F4A85A] text-[#1B2A41]' : 'bg-white/10 text-white/50'
                  }`}>
                    {idx + 1}
                  </div>
                  <span className={`mt-2 text-[10px] font-semibold ${idx === currentStep ? 'text-white' : 'text-white/60'}`}>
                    {step.label}
                  </span>
                </div>
              ))}
              ...
              <div 
                className="absolute top-4 left-0 h-[2px] bg-[#F4A85A] transition-all duration-1000 -z-0" 
            ...
            <div className="bg-black/30 rounded-xl p-4 font-mono text-sm text-[#F4A85A] min-h-[60px] border border-white/5">
              {displayText}
              <span className="inline-block w-2 h-4 bg-[#F4A85A] ml-1 animate-pulse align-middle"></span>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={executeLogin} className="space-y-5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input name="username" value={formData.username} onChange={handleInputChange} className="w-full bg-[#EEF3F6] rounded-lg p-3 text-sm outline-none focus:ring-2 ring-[#F4A85A] border border-transparent" required />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Contraseña</label>
                  <input name="password" type="password" value={formData.password} onChange={handleInputChange} className="w-full bg-[#EEF3F6] rounded-lg p-3 text-sm outline-none focus:ring-2 ring-[#F4A85A] border border-transparent" required />
                </div>
              </div>
            ...
            <form onSubmit={executeRegister} className="space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input name="email" type="email" value={formData.email} onChange={handleInputChange} className="w-full bg-[#EEF3F6] rounded-lg p-3 text-sm outline-none focus:ring-2 ring-[#F4A85A] border border-transparent" required />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Contraseña</label>
                  <input name="password" type="password" value={formData.password} onChange={handleInputChange} className="w-full bg-[#EEF3F6] rounded-lg p-3 text-sm outline-none focus:ring-2 ring-[#F4A85A] border border-transparent" required />
